...
#need to have the python file in its own folder
...
print(Input_report_timesheet +"/test.xlsx")
# create a dataframe 
df= pd.read_excel(Input_report_timesheet +"/test.xlsx")
...
if len(companies_in_timesheet_copy) > 0:
    ...
    for company in timesheets_df['location'].unique():
        ...
        for index in indices_of_armed_guards:
            print('armed',temp_company_df.loc[index,'rate'])
            print('armed',company.lower().rstrip(' ').lstrip(' '),' ',armed_guard_dict[company.lower().rstrip(' ').lstrip(' ')])
            temp_company_df.loc[index,'rate'] = armed_guard_dict[company.lower().rstrip(' ').lstrip(' ')]
            print('armed after',temp_company_df.loc[index,'rate'])
        ...
        print('supervisor indices',indices_of_supervisors)

        #charges the supervisor rate for the supervisors
        for index in indices_of_supervisors:
            print('index',index)
            print('supervisor',temp_company_df.loc[index,'rate'])
            print('supervisor',company.lower().rstrip(' ').lstrip(' '),' ',supervisor_dict[company.lower().rstrip(' ').lstrip(' ')])
            temp_company_df.loc[index,'rate'] = supervisor_dict[company.lower().rstrip(' ').lstrip(' ')]
            print('supervisor after',temp_company_df.loc[index,'rate'])
